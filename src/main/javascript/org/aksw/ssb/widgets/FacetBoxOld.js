...
(function($) {
	...
	ns.FacetValueBackendSparql = function(sparqlService, queryGenerator) {
		this.sparqlService = sparqlService;
		this.queryGenerator = queryGenerator;
	};

	ns.FacetValueBackendSparql.prototype.fetchFacetValues = function(
			path, facetState, searchString) {
		
		var queryGenerator = this.queryGenerator.copyExcludeConstraints(path);
		
		return queryUtils.fetchFacetValues(this.sparqlService, this.facetConfig,
				this.queryGenerator, path, searchString);
	...
	/**
	 * @Deprecated
	 * This class is was only a QueryGenerator in disguise. Use a query generator instead.
	 * 
	 ...
	 */
	/*
	ns.FacetState = function(queryGenerator, path) {
		//this.config = config;
		this.queryGenerator = queryGenerator;
		this.path = path;
	...
	*/
	...
	ns.createFacetBox = function(config, queryGenerator, basePath, backend, callbacks) {

		var facetList = ns.createFacetList(config, queryGenerator, basePath, backend,
				callbacks);
	 ...
	 */
	ns.createFacetList = function(config, queryGenerator, basePath, backend, callbacks) {
